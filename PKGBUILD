# Maintainer: ._______166
pkgname=dot166-metapackage
pkgver=4
pkgrel=4
pkgdesc="metapackage for common applications and dotfiles"
arch=('x86_64')
license=('GPL')
install=$pkgname.install
depends=('grapheneos-devel' 'git' 'vim' 'android-studio' 'github-cli' 'jdk17-openjdk' 'ttf-comic-sans' 'hyfetch' 'fastfetch' 'pipewire-pulse' 'maliit-keyboard' 'linux-headers' 'libreoffice-fresh-en-gb' 'hunspell' 'hunspell-en_gb' 'gnome-calculator' 'firefox' 'zsh' 'vi-vim-symlink' 'gnome-characters' 'gnome-console' 'gnome-disk-utility' 'gnome-font-viewer' 'qt5-wayland' 'gnome-system-monitor' 'signal-desktop' 'ntfs-3g' 'gnome-text-editor' 'rustrover' 'rustrover-jre' 'sddm' 'hyprland' 'hyprpaper' 'waybar' 'wofi' 'eww' 'ttf-jetbrains-mono-nerd' 'xdg-desktop-portal-hyprland' 'cargo' 'nwg-look' 'network-manager-applet' 'dunst' 'hyprshot' 'otf-font-awesome')
makedepends=()
#source=("$pkgname-$pkgver.tar.gz"
#"$pkgname-$pkgver.patch")

#prepare() {
#cd "$pkgname-$pkgver"
#patch -p1 -i "$srcdir/$pkgname-$pkgver.patch"
#}

#build() {
#cd "$pkgname-$pkgver"
#./configure --prefix=/usr
#make
#}

#package() {
#  install -dm777 "$srcdir/dots" "$HOME"
#}
